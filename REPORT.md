...
## 実装済み: B+F ハイブリッドパイプライン

フォトグラメトリで得たSTLを「正解」とし、build123d生成モデルと比較しながら
Claude Code で反復改善するパイプラインを実装した。
...
[Phase 2: フォトグラメトリ]
  写真群 → COLMAP / Meshroom → scan.stl (正解メッシュ)

[Phase 3: 比較ループ]  ← ここを自動化済み
  ┌─────────────────────────────────────────────┐
  │  make compare-saito-fa-125-engine           │
  │       STL=scan/saito-fa-125-engine.stl      │
  │                                              │
  │  出力:                                       │
  │    compare_*.png  → 並列比較画像             │
  │    overlay_*.png  → 半透明重ね合わせ         │
  │    dimensions.txt → 寸法差分レポート         │
  │                                              │
  │  Claude Code が dimensions.txt を読み        │
  │  model/*.py のパラメータを修正               │
  │                                              │
  │  → 再度 make compare-... → 差分が縮小       │
  └─────────────────────────────────────────────┘
```
...
| ファイル | 内容 | 用途 |
|----------|------|------|
| `compare_isometric.png` | 左右並列 (赤=スキャン, 青=生成) | 全体形状の目視比較 |
| `compare_front.png` | 正面ビュー並列 | プロペラハブ周りの確認 |
| `compare_side.png` | 側面ビュー並列 | シリンダー/フィン高さの確認 |
| `compare_top.png` | 上面ビュー並列 | ヘッド形状の確認 |
| `overlay_*.png` | 半透明重ね合わせ | はみ出し/不足の直接比較 |
| `dimensions.txt` | 寸法差分テーブル | Claude Code の修正根拠 |

### dimensions.txt の読み方

```
--- 全体寸法 (mm) ---
                     Reference    Generated       差分     比率
X (前後)                98.30       102.00     +3.70  103.76%  ← 3.7mm 長すぎ
Y (左右)                55.20        54.00     -1.20   97.83%  ← ほぼOK
Z (上下)               108.50       106.40     -2.10   98.06%  ← 2.1mm 短い

--- Z方向断面寸法 ---
Z=  -20.0   Ref幅X=52.00  Gen幅X=55.00  差=+3.00   ← クランクケースが太い
Z=   30.0   Ref幅X=50.00  Gen幅X=52.00  差=+2.00   ← フィンがやや大きい
```

「差分」列の値を見て、対応するパラメータ（cc_len, fin_d, etc.）を調整する。

### 撮影ガイド (Saito FA-125 の場合)

```
推奨撮影条件:
  - カメラ: スマホ可（ただしマクロ寄りで撮ること）
  - 枚数:  60-100枚
  - 配置:  エンジンを回転台に固定、カメラを周回
  - 高度:  3段階（水平 / 斜め45° / 真上付近）× 各20-30枚
  - 照明:  拡散光（直射日光・スポットライトNG）
  - 背景:  単色マット（白い紙等）
  - 注意:  アルミの鏡面反射を抑える（マットスプレー推奨）
...
# 1. スキャンSTLを配置
mkdir -p scan
cp /path/to/photogrammetry_output.stl scan/saito-fa-125-engine.stl

# 2. 比較実行
make compare-saito-fa-125-engine STL=scan/saito-fa-125-engine.stl

# 3. Claude Code に改善を依頼
#    「out/saito-fa-125-engine/dimensions.txt と overlay_*.png を見て
#     model/saito-fa-125-engine.py を修正して」

# 4. 再比較 → 差分が許容範囲になるまで繰り返し
make compare-saito-fa-125-engine STL=scan/saito-fa-125-engine.stl
```
